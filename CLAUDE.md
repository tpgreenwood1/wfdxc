...
`spec.md` is the design source of truth (data model, scoring rules, lifecycle) — read it for anything not obvious from the code. It also lists still-open decisions (points-for-place vs sum-of-positions, etc.) that aren't yet resolved in code.

- **Next.js App Router**, three surfaces: `/submit/[token]` (tokenised teacher entry form, no login), `/admin/*` (scorer admin), and public read-only `/results/[id]` + `/standings`.
...
- Business logic lives in `lib/*.ts`, not inline in route handlers/actions: `runners.ts` (fuzzy search, quick-add, rename, merge — all pg_trgm-backed), `races.ts`, `results.ts`, `events.ts`, `tokens.ts`, `scoring.ts`, `standings.ts`, `standings-query.ts`, `publish.ts`, `public-results.ts`.
